...
from torch_mimicry.modules import SNConv1d, ConditionalBatchNorm1d
...
class GBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if self.spectral_norm:
            self.c1 = SNConv1d(self.in_channels,
                               self.hidden_channels,
                               9,
                               1,
                               padding=1)
            self.c2 = SNConv1d(self.hidden_channels,
                               self.out_channels,
                               9,
                               1,
                               padding=1)
        ...
        if self.num_classes == 0:
            ...
            self.b1 = ConditionalBatchNorm1d(self.in_channels,
                                             self.num_classes)
            self.b2 = ConditionalBatchNorm1d(self.hidden_channels,
                                             self.num_classes)
        ...
        if self.learnable_sc:
            if self.spectral_norm:
                self.c_sc = SNConv1d(in_channels,
                                     out_channels,
                                     1,
                                     1,
                                     padding=0)
...
class DBlock(nn.Module):
    ...
    def __init__(self,
        ...
        # Build the layers
        if self.spectral_norm:
            self.c1 = SNConv1d(self.in_channels, self.hidden_channels, 9, 1, 1)
            self.c2 = SNConv1d(self.hidden_channels, self.out_channels, 9, 1,
                               1)
        else:
            self.c1 = nn.Conv1d(self.in_channels, self.hidden_channels, 9, 1,
                                1)
            self.c2 = nn.Conv1d(self.hidden_channels, self.out_channels, 9, 1,
                                1)
        ...
        if self.learnable_sc:
            if self.spectral_norm:
                self.c_sc = SNConv1d(in_channels, out_channels, 1, 1, 0)
            else:
...
class DBlockOptimized(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, spectral_norm=True):
        ...
        # Build the layers
        if self.spectral_norm:
            self.c1 = SNConv1d(self.in_channels, self.out_channels, 9, 1, 1)
            self.c2 = SNConv1d(self.out_channels, self.out_channels, 9, 1, 1)
            self.c_sc = SNConv1d(self.in_channels, self.out_channels, 1, 1, 0)
        else:
            self.c1 = nn.Conv1d(self.in_channels, self.out_channels, 9, 1, 1)
            self.c2 = nn.Conv1d(self.out_channels, self.out_channels, 9, 1, 1)
