...
class SelfAttention(nn.Module):
    ...
    def __init__(self, num_feat, spectral_norm=True):
        ...
        if self.spectral_norm:
            self.theta = SNConv1d(self.num_feat,
                                  self.num_feat >> 3,
                                  1,
                                  1,
                                  padding=0,
                                  bias=False)
            self.phi = SNConv1d(self.num_feat,
                                self.num_feat >> 3,
                                1,
                                1,
                                padding=0,
                                bias=False)
            self.g = SNConv1d(self.num_feat,
                              self.num_feat >> 1,
                              1,
                              1,
                              padding=0,
                              bias=False)
            self.o = SNConv1d(self.num_feat >> 1,
                              self.num_feat,
                              1,
                              1,
                              padding=0,
            ...
            self.theta = nn.Conv1d(self.num_feat,
                                   self.num_feat >> 3,
                                   1,
                                   1,
                                   padding=0,
                                   bias=False)
            self.phi = nn.Conv1d(self.num_feat,
                                 self.num_feat >> 3,
                                 1,
                                 1,
                                 padding=0,
                                 bias=False)
            self.g = nn.Conv1d(self.num_feat,
                               self.num_feat >> 1,
                               1,
                               1,
                               padding=0,
                               bias=False)
            self.o = nn.Conv1d(self.num_feat >> 1,
                               self.num_feat,
                               1,
                               1,
                               padding=0,
    ...
    def forward(self, x):
        ...
        # Phi path
        phi = self.phi(x)
        phi = F.max_pool1d(phi, 2, stride=2)
...
def SNConv1d(*args, default=True, **kwargs):
    r"""
    Wrapper for applying spectral norm on conv1d layer.
    """
    if default:
        return nn.utils.spectral_norm(nn.Conv1d(*args, **kwargs))

    else:
        return spectral_norm.SNConv1d(*args, **kwargs)
